from imsu import common_tools, db_queries, population_generator, lc_generator, recovery, plotting_suite
import shutil
import sys
import os
import json
...
def main():
	...
	shutil.copy('settings.yaml', os.path.join(output_path, 'settings.yaml'))
	...
	if program_settings['Generate Lightcurves']:
		...
		elif lightcurve_settings['population']['transient type'] == 'WDNS':
			
			source = lc_generator.addTimeSeriesSource(obj_name = 'at2018kzr', explosion_epoch = 2458477.922, redshift = 0.05298, min_wl = 3400., max_wl = 12000, npoints = 10000)
			lc_population = lc_generator.generateSNLightcurvePopulation(query_output, population_settings, lightcurve_settings, io_settings, source)
	...
	if program_settings['Perform Recovery']:
		...
		lc_population = recovery.initialiseRecoveryProcedure(lc_population)
		...
		population_settings = recovery.markRecoveriesByFilter(lc_population, recovery_settings, population_settings)
		print(population_settings)
		
		recovery.calculateEfficiencies(recovery_settings, population_settings)
			
		recovery.saveRecoveryData(lc_population, population_settings, recovery_lc_path, recovery_pop_path)
		
	else:
		
		if not os.path.exists(recovery_lc_path):
			sys.exit('Lightcurve recovery has not been performed. Enable recovery module.')
		else:
			with open(recovery_lc_path) as f:
				lc_population = json.load(f)
			
			population_settings = pd.read_csv(recovery_pop_path)
...
# 		plotting_suite.plotLimitingMagnitudesFromQuery(query_output, lightcurve_settings)
		...
		plotting_suite.makeExplosionEpochHistogram(population_settings, the_survey, lightcurve_settings)
		
		plotting_suite.make2DHistogramRecoveredPointings(population_settings, lightcurve_settings)
